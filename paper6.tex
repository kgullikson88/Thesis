...
\chapter{The Inner Mass-Ratio Distribution of Intermediate-Mass Stars\footnote{
    This chapter has been submitted for publication to the Astronomical Journal, but is not yet published.
}}
\label{chap:survey}
...
For the optical data, we use the blue \'echelle orders ($\lambda < 5550 \AA$). We ignore the strong hydrogren Balmer lines in the 
spectrum because they span several \'echelle orders and make continuum normalization very difficult, potentially biasing the 
result. There are sufficient metal lines in the optical spectra that the resulting CCF always has a very strong peak at the radial 
velocity of the primary star. The near-infrared IGRINS spectra have very few strong metal lines; we use the subset from $1.51 - 
1.73 \mu m$ that is dominated by hydrogren Brackett lines for these spectra. Similar to the companion search, we estimate the 
temperature and surface gravity of the stars from the CCF with the largest peak. We adopt the following errors on the temperature 
and surface gravity, which are based on the grid step size and are somewhat more pessimistic than typical uncertainties seen in 
the literature for A- and B-type stars \citep[e.g.][]{Aydi2014, David2015}:
...
\begin{longtable}{|l|llllllll|}
    ...
    \caption{NIRI Observation log. \label{paper6_tab:imaging_obs}}
    \\ \hline
...
\begin{longtable}{|l|rrlrrrll|}
    ...
    \caption{Companion Detections. \label{paper6_tab:companions}}
    \\ \hline
...
\afterpage{%
\begin{normalsize}
\begin{longtable}{|r|rlll|}
    
    \caption{Mass-Ratio Distribution Parameters. \label{paper6_tab:parameters}} \\ \hline
       & & & \multicolumn{2}{c}{$1 \sigma$ credible interval} \\
      Model & Parameter & Median Value & Lower & Upper \\ \hline
    \endfirsthead
